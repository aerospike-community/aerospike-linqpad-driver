...
*/
aerospike_cloud.Customer.Where(c => c.Country == "USA" && c.State == "CA").Dump("Using Linq");
aerospike_cloud.Customer.Query(Exp.And(Exp.EQ(Exp.StringBin("Country"), Exp.Val("USA")), Exp.EQ(Exp.StringBin("State"), Exp.Val("CA")))).Dump("Using Aerospike Expressions");